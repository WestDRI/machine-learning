...
There are now more types of learning than those presented here. But these initial types are interesting because they will be familiar to you already (even if you didn't look at them in the framework of machine learning).
...
Deep neural networks have 2 or more hidden layers (shallow networks only have one hidden layer). When an ANN is deep, we talk about *Deep Learning* (DL). 
