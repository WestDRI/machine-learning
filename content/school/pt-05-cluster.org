...
Throughout your script, make sure to have plots written to file and not displayed to screen.

* Prepare the data

The Compute Canada clusters are optimized for very large files and are slowed by large collections of small files. Datasets with many small files need to be turned into single large files with {{<c>}}tar{{</c>}}. Failing to do so will affect performance not just for you, but for all users of the cluster.
...
#+BEGIN_src sh
[local]$ scp </local/path/file>  user<xxx>@cassiopeia.c3.ca:<path/cluster>
#+END_src
...
#+BEGIN_src sh
[cluster]$ scp user<xxx>@cassiopeia.c3.ca:<cluster/path/file>  </local/path>
#+END_src
