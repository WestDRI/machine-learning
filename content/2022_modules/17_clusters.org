...
Use [[https://docs.alliancecan.ca/wiki/Globus][Globus]] for large data transfers.

Note that the Alliance is planning to store classic ML datasets on its clusters in the future. So if your research uses a common dataset, it may be worth inquiring whether it might be available before downloading a copy.

** Large collections of files

The Alliance clusters are optimized for very large files and are slowed by large collections of small files. Datasets with many small files need to be turned into single-file archives with {{<c>}}tar{{</c>}}. *Failing to do so will affect performance not just for you, but for all users of the cluster.*
...
- As a modern alternative to {{<c>}}tar{{</c>}}, you can use [[https://docs.alliancecan.ca/wiki/Dar][Dar]]
#+END_note
...
#+BEGIN_note
Be aware that, on Cedar, [[https://docs.alliancecan.ca/wiki/Running_jobs#Cluster_particularities][you are not allowed to submit jobs from ~/home~]]. Instead, you have to submit jobs from ~/scratch~ or ~/project~.
#+END_note
...
- There are various other options for [[https://docs.alliancecan.ca/wiki/Running_jobs#Email_notification][email notifications]]
#+END_note
...
From <a href="https://docs.alliancecan.ca/wiki/Using_GPUs_with_Slurm">the Alliance Wiki</a>
<br><br>
{{</img>}}

The default is {{<b>}}12G P100{{</b>}}, but you can request another type with {{<c>}}SBATCH --gres=gpu:&lt;type&gt;:&lt;number&gt;{{</c>}} (example: {{<c>}}--gres=gpu:p100l:1{{</c>}} to request a 16G P100 on Cedar). Please [[https://docs.alliancecan.ca/wiki/Using_GPUs_with_Slurm#Specifying_the_type_of_GPU_to_use][refer to the Alliance Wiki]] for more details.
...
/(Replace {{<c>}}&lt;node hostname&gt;{{</c>}} by the compute node hostname you just identified, {{<c>}}&lt;user&gt;{{</c>}} by your user name, and {{<c>}}&lt;cluster&gt;{{</c>}} by the name of the Alliance cluster hostname—e.g. beluga, cedar, graham.)/
